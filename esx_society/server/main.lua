...
AddEventHandler('esx_society:checkSocietyBalance', function(society)
	...
	local _society = GetSociety(society)

	if not xPlayer or not _society then return end

	if xPlayer.job.name ~= _society.name then
		print(('esx_society: %s attempted to call checkSocietyBalance!'):format(xPlayer.identifier))
		return
	end

	TriggerEvent('esx_addonaccount:getSharedAccount', _society.account, function(account)
		if account then 
			TriggerClientEvent('ox_lib:notify', xPlayer.source, {
				title = _society.label,
				description = TranslateCap('check_balance', ESX.Math.GroupDigits(account.money)),
				type = 'inform',
				duration = 5000,
			})
		end
...
AddEventHandler('esx_society:washMoney', function(society, amount)
	...
	if not xPlayer or not amount or not tonumber(amount) then return end
	amount = ESX.Math.Round(tonumber(amount))
